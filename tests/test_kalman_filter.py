...
kf_path = os.path.join(
    os.getcwd(), "UTILS - Quantitative Methods - Kalman Filter", "kalman_filter.py"
)
...
class TestKalmanFilter(unittest.TestCase):
    def test_initialization(self):
        kf = kf_module.KalmanFilter1D(
            process_variance=0.1, measurement_variance=0.1, estimated_value=10
        )
    ...
    def test_convergence(self):
        ...
        target = 100.0
        kf = kf_module.KalmanFilter1D(
            process_variance=1e-5, measurement_variance=0.1, estimated_value=50.0
        )
